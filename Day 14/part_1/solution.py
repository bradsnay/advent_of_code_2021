...
def apply_rules(polymer_template: str, pair_insertion_rules: dict) -> Tuple[str, int, int]:
    new_polymer_template = ""
    character_counts = Counter()
    for i in range(len(polymer_template)):
        char = polymer_template[i]
        character_counts[char] += 1
        pair = f"{char}{polymer_template[i+1]}" if i+1 < len(polymer_template) else None
    ...
    return new_polymer_template, max(character_counts.values()), min(character_counts.values())
...
for i in range(10):
    polymer_template, max_character_count, min_character_count = apply_rules(polymer_template, pair_insertion_rules)
