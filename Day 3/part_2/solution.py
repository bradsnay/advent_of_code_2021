...
def count_column_values(binary_input: set, bit_index: int):
    column_value_counts = {}
    column_value_values = {}
    for value in binary_input:
        bit = int(value[bit_index])
        if column_value_counts.get(bit_index) is None:
            column_value_counts[bit_index] = {}
            column_value_values[bit_index] = {}
        if column_value_counts[bit_index].get(bit) is None:
            column_value_counts[bit_index][bit] = 0
            column_value_values[bit_index][bit] = set()
        column_value_counts[bit_index][bit] += 1
        column_value_values[bit_index][bit].add(value)
    return column_value_counts, column_value_values


def value_to_remove_oxygen_sensor(zero_count: int, one_count: int):
    return 0 if one_count >= zero_count else 1


def value_to_remove_co2_scrubber(zero_count: int, one_count: int):
    return 0 if one_count < zero_count else 1
...
oxygen_rating = apply_bit_criteria(file_input, value_to_remove_oxygen_sensor)
co2_rating = apply_bit_criteria(file_input, value_to_remove_co2_scrubber)
final_result = int(oxygen_rating, 2) * int(co2_rating, 2)
print(final_result)
