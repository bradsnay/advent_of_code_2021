...
def perform_single_fold(
    should_fold_y: bool, axis_fold_value: int, coordinates: set, max_x: int, max_y: int
) -> Tuple[set, int, int]:
    new_coordinates = set()
...
def perform_folds(
    folding_instructions: list, coordinates: set, max_x: int, max_y: int, num_times: int
):
    iterations = 0
    for axis, axis_value in folding_instructions:
        if iterations == num_times:
            break
        coordinates, max_x, max_y = perform_single_fold(
            axis == "y", axis_value, coordinates, max_x, max_y
        )
        iterations += 1
    ...
    for line in file:
        ...
        if line.startswith("fold"):
            ...
            folding_instructions.append(
                (folding_instruction[0], int(folding_instruction[1]))
            )
            pass
...
new_coordinates, max_x, max_y = perform_folds(
    folding_instructions, dot_coordinates, max_x, max_y, 1
)
