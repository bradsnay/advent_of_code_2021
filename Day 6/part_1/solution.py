...
class LanternFish:
    def __init__(self, internal_timer_init: int):
        self.internal_timer = internal_timer_init

    def pass_a_day(self) -> Union["LanternFish", None]:
...
for day in range(0, num_days_to_simulate):
    print_str = f"Day {day}: "
    lantern_fishes_to_append = []
    for lantern_fish in lantern_fishes:
        # print_str = f"{print_str} {lantern_fish.internal_timer}"
        new_lantern_fish = lantern_fish.pass_a_day()
        if new_lantern_fish is not None:
            lantern_fishes_to_append.append(new_lantern_fish)
    # print(print_str)
    lantern_fishes.extend(lantern_fishes_to_append)

# We calculated the final day but didn't print it. Print it here
# print_str = f"Day {num_days_to_simulate}: "
# for lantern_fish in lantern_fishes:
#     print_str = f"{print_str} {lantern_fish.internal_timer}"
# print(print_str)
